...
static GLuint make_text_texture(const char* text, int wrap_w_pixels, int* out_w, int* out_h){
    ...
    SDL_Surface* surf = TTF_RenderText_Blended_Wrapped(g_font, text, (int)SDL_strlen(text), white, wrap_w_pixels);
    if (!surf) {
...
static void draw_tex_screen(const AmeCamera* cam, int viewport_w, int viewport_h,
                            GLuint tex, int px_w, int px_h, float px_x, float px_y){
    if (!tex || !cam) return;
...
static void ensure_font_for_viewport(int viewport_h){
    ...
    if (scale < 0.5f) scale = 0.5f; if (scale > 2.0f) scale = 2.5f;
    int desired_px = (int)(18.0f * scale);
    if (desired_px < 12) desired_px = 12;
    if (desired_px > 48) desired_px = 48;
    if (g_font && g_loaded_font_px == desired_px) return;
...
void ui_shutdown(void){
    if (g_font){ TTF_CloseFont(g_font); g_font = NULL; }
    if (g_hud_texture) { glDeleteTextures(1, &g_hud_texture); g_hud_texture = 0; }
    if (g_dialogue_texture) { glDeleteTextures(1, &g_dialogue_texture); g_dialogue_texture = 0; }
    g_loaded_font_px = 0; g_font_path[0] = '\0';
    g_last_hud_text[0] = '\0'; g_last_dialogue_text[0] = '\0';
    TTF_Quit();
}
...
void ui_render_hud(const AmeCamera* cam, int viewport_w, int viewport_h, const Car* car){
    ...
    char hud[128];
    SDL_snprintf(hud, sizeof(hud), "HP: %.0f/%.0f    Fuel: %.0f/%.0f", car->hp, car->max_hp, car->fuel, car->max_fuel);
    ...
    if (SDL_strcmp(hud, g_last_hud_text) != 0) {
        ...
        g_hud_texture = make_text_texture(hud, viewport_w - (int)(2*margin), &g_hud_tw, &g_hud_th);
...
void ui_render_dialogue(const AmeCamera* cam,
    ...
    if (ln){
        if (ln->speaker && ln->speaker[0]){ SDL_strlcat(buf, ln->speaker, sizeof(buf)); SDL_strlcat(buf, ": ", sizeof(buf)); }
        if (ln->text) SDL_strlcat(buf, ln->text, sizeof(buf));
        if (ln->option_count > 0){
            SDL_strlcat(buf, "\n\n", sizeof(buf));
            for (size_t i=0;i<ln->option_count && i<9;i++){
                char line[256]; SDL_snprintf(line, sizeof(line), "%zu) %s\n", i+1, ln->options[i].choice ? ln->options[i].choice : "");
    ...
    if (g_dialogue_texture && g_dialogue_texture != 0){
        ...
        draw_tex_screen(cam, viewport_w, viewport_h, g_dialogue_texture, g_dialogue_tw, g_dialogue_th, viewport_w/2.0f, y+viewport_h/10.0f);
    }
}
