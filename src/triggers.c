...
static inline int aabb_overlap(Aabb a, Aabb b){
  float ax0=a.x - a.w*0.5f, ay0=a.y - a.h*0.5f, ax1=a.x + a.w*0.5f, ay1=a.y + a.h*0.5f;
  float bx0=b.x - b.w*0.5f, by0=b.y - b.h*0.5f, bx1=b.x + b.w*0.5f, by1=b.y + b.h*0.5f;
  return !(ax1 < bx0 || bx1 < ax0 || ay1 < by0 || by1 < ay0);
}

void triggers_init(void){ g_count = 0; memset(g_triggers, 0, sizeof(g_triggers)); }
void triggers_clear(void){ g_count = 0; }

void triggers_add(const char* name, Aabb box, int once, TriggerCallback cb, void* user){
  if(g_count >= MAX_TRIGGERS) return;
...
void triggers_update(float player_x, float player_y, float player_w, float player_h,
  ...
  for(int i=0;i<g_count;i++){
    if(g_triggers[i].once && g_triggers[i].fired) continue;
    int hit = aabb_overlap(g_triggers[i].box, pa) || aabb_overlap(g_triggers[i].box, ca);
    if(hit){
      if(g_triggers[i].cb){ g_triggers[i].cb(g_triggers[i].name, g_triggers[i].user); }
...
void triggers_fire(const char* name){
  ...
  for(int i=0;i<g_count;i++){
    if(g_triggers[i].name && SDL_strcmp(g_triggers[i].name, name)==0){
      if(g_triggers[i].once && g_triggers[i].fired) return;
      if(g_triggers[i].cb){ g_triggers[i].cb(g_triggers[i].name, g_triggers[i].user); }
      g_triggers[i].fired = 1; return;
    }
  }
}
