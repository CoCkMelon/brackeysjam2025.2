...
static void
explosion_effect(float x, float y, float radius, float dmg, float impulse, Human* human, Car* car) {
        ...
        for (int i = 0; i < MAX_EXPLOSION_SOURCES; i++) {
            if (!g_expl_pool[i].active) {
                ...
                const char* candidates[] = {"assets/sfx/explosion.opus", "assets/explosion.opus",
                                            "/home/melony/llmgen/a_mongoose_engine/examples/"
                                            "audio_ray_example/explosion.opus",
                                            NULL};
                for (int k = 0; candidates[k]; ++k) {
                    if (ame_audio_source_load_opus_file(&g_expl_pool[i].src, candidates[k], false))
                        break;
                }
...
static int first_free_grenade(void) {
    for (int i = 0; i < MAX_GRENADES; i++)
        if (!grenades[i].alive)
            return i;
    return -1;
}
static int first_free_mine(void) {
    for (int i = 0; i < MAX_MINES; i++)
        if (!mines_[i].alive)
            return i;
    return -1;
}
static int first_free_turret(void) {
    for (int i = 0; i < MAX_TURRETS; i++)
        if (!turrets[i].alive)
            return i;
    return -1;
}
static int first_free_rocket(void) {
...
bool gameplay_init(void) {
    ...
    tex_grenade = load_texture_once_local("assets/Cherry.png");
    tex_mine = load_texture_once_local("assets/CookieMine.png");
    tex_turret = make_color_tex(80, 160, 80);
    tex_rocket = load_texture_once_local("assets/CookieRocket.png");
    ...
    const char* candidates[] = {
        "assets/sfx/explosion.opus", "assets/explosion.opus",
...
void gameplay_shutdown(void) {
    if (tex_grenade)
        glDeleteTextures(1, &tex_grenade);
    if (tex_mine)
        glDeleteTextures(1, &tex_mine);
    if (tex_turret)
        glDeleteTextures(1, &tex_turret);
    if (tex_rocket)
        glDeleteTextures(1, &tex_rocket);
}
...
static void mine_fixed_all(Human* human, Car* car) {
    for (int i = 0; i < MAX_MINES; i++) {
        if (!mines_[i].alive || !mines_[i].armed)
            continue;
        float hx = 0, hy = 0, cx = 0, cy = 0;
        if (human)
            human_get_position(human, &hx, &hy);
        if (car)
            car_get_position(car, &cx, &cy);
        float r = 16.0f;
        ...
        if (dxh * dxh + dyh * dyh < r * r || dxc * dxc + dyc * dyc < r * r) {
            explosion_effect(mines_[i].x, mines_[i].y, 50.0f, GAME_MINE_DAMAGE, 15000.0f, human,
                             car);
            mines_[i].alive = 0;
        }
    }
...
static void turret_fixed_all(Human* human, Car* car, float dt) {
    for (int i = 0; i < MAX_TURRETS; i++) {
        ...
        if (turrets[i].cooldown <= 0.0f) {
            ...
            float tx = 0, ty = 0;
            float bestd2 = 1e9f;
            float ox = turrets[i].x, oy = turrets[i].y;
            if (human) {
                float hx, hy;
                human_get_position(human, &hx, &hy);
                float dx = hx - ox, dy = hy - oy;
                float d2 = dx * dx + dy * dy;
                if (d2 < bestd2) {
                    bestd2 = d2;
                    tx = hx;
            ...
            if (bestd2 < 400.0f * 400.0f) {
                ...
                if (rc.hit && rc.body) {
                    if (human && rc.body == human->body) {
                        human_apply_damage(human, GAME_TURRET_SHOT_DAMAGE);
                    } else if (car && rc.body == car->body) {
                        car_apply_damage(car, GAME_TURRET_SHOT_DAMAGE);
                    }
                }
            } else {
...
static void rocket_fixed_all(Human* human, Car* car, float dt) {
    ...
    for (int i = 0; i < MAX_ROCKETS; i++) {
        ...
        if (rockets[i].life <= 0.0f) {
            float rx = 0, ry = 0;
            physics_get_position(rockets[i].body, &rx, &ry);
        ...
        float vx, vy;
        physics_get_velocity(rockets[i].body, &vx, &vy);
...
void spawn_fuel_pickup(float x, float y, float amount) {
    for (int i = 0; i < MAX_FUEL; i++) {
        if (!fuels[i].alive) {
            ...
            fuels[i].x = x;
            fuels[i].y = y;
            fuels[i].amount = amount > 0 ? amount : 25.0f;
            break;
        }
    }
}
...
static inline int
aabb_overlap(float ax, float ay, float aw, float ah, float bx, float by, float bw, float bh) {
...
void gameplay_fixed(Human* human, Car* car, float dt) {
    ...
    for (int i = 0; i < saw_count; i++) {
        if (saws[i].cut_cooldown > 0.0f)
            saws[i].cut_cooldown -= dt;
    ...
    for (int i = 0; i < saw_count; i++) {
        if (!saws[i].alive || !saws[i].body)
            continue;
        physics_set_angular_velocity(saws[i].body, saws[i].ang_vel);
        float sx, sy;
        physics_get_position(saws[i].body, &sx, &sy);
        float rr = saws[i].r;
        // Human
        if (human && human->body) {
            ...
            if (touching) {
                float hvx, hvy;
                physics_get_velocity(human->body, &hvx, &hvy);
                ...
                float imp = 0.0f;
                if (physics_bodies_contact_speed(human->body, saws[i].body, &imp)) {
                    if (imp > GAME_SAW_IMPACT_THRESH) {
                        dps += (imp - GAME_SAW_IMPACT_THRESH) * GAME_SAW_IMPACT_SCALE_HUM;
                    }
                }
        ...
        if (car && car->body) {
            ...
            if (touching) {
                float cvx, cvy;
                physics_get_velocity(car->body, &cvx, &cvy);
                float rel = sqrtf(cvx * cvx + cvy * cvy) + fabsf(saws[i].ang_vel) * rr;
                float dps = fmaxf(GAME_SAW_BASE_DPS_CAR, (rel - 2.0f) * 1.2f);
                float imp = 0.0f;
                if (physics_bodies_contact_speed(car->body, saws[i].body, &imp)) {
                    if (imp > GAME_SAW_IMPACT_THRESH) {
                        dps += (imp - GAME_SAW_IMPACT_THRESH) * GAME_SAW_IMPACT_SCALE_CAR;
                    }
                }
    ...
    if (human && human->body) {
        float sp = 0.0f;
        if (physics_body_touching_flag_speed(human->body, PHYS_FLAG_SPIKE, &sp)) {
            float thresh = GAME_SPIKE_HUMAN_THRESH;
            if (sp > thresh) {
                float dmg = (sp - thresh) * GAME_SPIKE_HUMAN_SCALE * dt;
                human_apply_damage(human, dmg);
            }
        }
    }
    if (car && car->body) {
        float sp = 0.0f;
        if (physics_body_touching_flag_speed(car->body, PHYS_FLAG_SPIKE, &sp)) {
            float thresh = GAME_SPIKE_CAR_THRESH;
            if (sp > thresh) {
                float dmg = (sp - thresh) * GAME_SPIKE_CAR_SCALE * dt;
                car_apply_damage(car, dmg);
            }
        }
    }
    ...
    if (((human && human->health.hp <= 0.0f) || (car && car->hp <= 0.0f)) && active_spawn >= 0) {
        ...
        if (car) {
            car_set_position(car, sx, sy);
            car->hp = car->max_hp;
            car->fuel = car->max_fuel;
        }
        if (human) {
            human_set_position(human, sx, sy + 20.0f);
            human->health.hp = human->health.max_hp;
        }
    }
}
...
void gameplay_update(Human* human,
    ...
    for (int i = 0; i < saw_count; i++) {
        ...
        float sx, sy;
        physics_get_position(saws[i].body, &sx, &sy);
        float pan = (sx - cam_x - half_w) / half_w;
        if (pan < -1.0f)
            pan = -1.0f;
        if (pan > 1.0f)
            pan = 1.0f;
    ...
    if (g_audio_ready) {
        ...
        for (int i = 0; i < MAX_EXPLOSION_SOURCES; i++) {
            ...
            if (g_expl_pool[i].ttl <= 0.0f) {
                g_expl_pool[i].active = 0;
                continue;
            }
            float pan = (g_expl_pool[i].x - cam_x - half_w) / half_w;
            if (pan < -1.0f)
                pan = -1.0f;
            if (pan > 1.0f)
                pan = 1.0f;
            RaycastCallback ray =
                physics_raycast(g_expl_pool[i].x, g_expl_pool[i].y, listener_x, listener_y);
            float base_gain = 0.9f;
            float gain = base_gain;
            if (ray.hit && ray.fraction < 0.98f) {
                gain *= 0.35f;
            }
...
void gameplay_on_trigger(const char* name, void* user) {
    ...
    // Dialogue start via prefix: "TriggerDialogue" with optional spaces and a scene name
    if (SDL_strncmp(name, "TriggerDialogue", 15) == 0) {
        const char* scene = name + 15;  // after prefix
        // Some exporters replace spaces with '_' or ':'; skip separators
        while (*scene == ' ' || *scene == '_' || *scene == ':' || *scene == '-')
            scene++;
        if (scene && scene[0]) {
            SDL_Log("Trigger: starting dialogue scene '%s'", scene);
            dialogue_start_scene(scene);
        } else {
            SDL_Log("TriggerDialogue missing scene name in trigger '%s'", name);
        }
        return;
    }
    ...
    if (SDL_strncmp(name, "TriggerGrenade", 14) == 0) {
        ...
        // Find nearest between current global car/human isn’t accessible here; we’ll simply shoot
        // to the right
        ...
        Human* human = NULL;
        Car* car = NULL;  // cannot access globals; approximate by applying to both by distance at
                          // update time
        (void)human;
        (void)car;
        // We will apply damage opportunistically in fixed step by placing an invisible rocket with
        // instant life; simpler: just radial small damage here For correctness per request, do
        // direct damage by choosing closer target through physics query. Sample both if available
        // via a simple overlap check radius
    }
}

void gameplay_add_spawn_point(float x, float y) {
    if (spawn_count < MAX_SPAWNS) {
        spawns[spawn_count].x = x;
        spawns[spawn_count].y = y;
        spawns[spawn_count].active = 0;
        spawn_count++;
        if (active_spawn < 0)
            active_spawn = 0;
    }
...
int gameplay_collect_audio_refs(AmeAudioSourceRef* out,
                                int max_refs,
                                float cam_x,
                                float cam_y,
                                float viewport_w_pixels,
...
void gameplay_render(void) {
    ...
    for (int i = 0; i < MAX_GRENADES; i++)
        if (grenades[i].alive) {
            float x = 0, y = 0;
            physics_get_position(grenades[i].body, &x, &y);
            pipeline_sprite_quad_rot(x, y, 6, 6, 0, tex_grenade, 1, 1, 1, 1);
        }
    // Mines
    for (int i = 0; i < MAX_MINES; i++)
        if (mines_[i].alive) {
            pipeline_sprite_quad_rot(mines_[i].x, mines_[i].y, 8, 4, 0, tex_mine, 1, 1, 1, 1);
        }
    // Turrets (visible and aimed)
    for (int i = 0; i < MAX_TURRETS; i++)
        if (turrets[i].alive) {
            pipeline_sprite_quad_rot(turrets[i].x, turrets[i].y, 12, 8, turrets[i].ang, tex_turret,
                                     1, 1, 1, 1);
        }
    // Spawn points
    for (int i = 0; i < spawn_count; i++) {
    ...
    for (int i = 0; i < MAX_ROCKETS; i++)
        if (rockets[i].alive) {
            float x = 0, y = 0;
            physics_get_position(rockets[i].body, &x, &y);
            pipeline_sprite_quad_rot(x, y, 6, 3, 0, tex_rocket, 1, 1, 1, 1);
        }
    // Fuel pickups
    for (int i = 0; i < MAX_FUEL; i++)
        if (fuels[i].alive) {
            pipeline_sprite_quad_rot(fuels[i].x, fuels[i].y, 8, 8, 0, tex_fuel, 1, 1, 1, 1);
        }
}
